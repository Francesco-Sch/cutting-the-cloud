...
From two of the chips the label was earsed, so I could not find the exact price for them. Nevertheless, these chips are probably a TuyaMCU chip as well as an RTC chip. Some other components do have a label but could not be identified. [^1]
...
As though this is a very rough estimate, as I could not find the exact components, I think it is a good starting point to get a feeling for the value of just the components.
Substracted from the original price of the device, it leaves us with:

{/* Add LaTeX calculation */}

From this remaining amount there still needs to be covered cost such as

- woiubni

This is only the COGS (Cost of Goods) and Markup is not even included. Eventhough this is a very sketchy calculation it gives a good starting to point to estimate how much the labour was valued that went into the production of this device.
